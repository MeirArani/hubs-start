...
import { type EntityState } from './entity-state-utils';
...
export function listenForNetworkMessages(
  channel: PhoenixChannel,
  presenceEventEmitter: Emitter,
) {
...
function onJoin({ key }: { key: ClientID }) {
  const clientId = window.APP.getSid(key);
