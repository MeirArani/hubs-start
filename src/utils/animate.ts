  ...
  while (now < end) {
    ...
    for (let i = 0; i < values.length; i++) {
      if (typeof values[i] === 'number') {
        values[i] = MathUtils.lerp(
          properties[i][0] as number,
          properties[i][1] as number,
          t,
        );
      } else {
        (values[i] as Vector3).lerpVectors(
          properties[i][0] as Vector3,
          properties[i][1] as Vector3,
          t,
        );
      }
    }
