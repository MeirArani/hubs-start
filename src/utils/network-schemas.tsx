...
export const schemas: Map<ComponentRef, NetworkSchema> = new Map();
...
export function read(prop: any, eid: EntityID) {
  if (ArrayBuffer.isView(prop[eid])) {
    ...
    return prop[$isStringType] ? window.APP.getString(prop[eid]) : prop[eid];
  }
}
...
export function write(prop: any, eid: EntityID, value: any) {
  if (ArrayBuffer.isView(prop[eid])) {
    ...
    prop[eid] = prop[$isStringType] ? window.APP.getString(value) : value;
  }
}
...
export function deserializerWithMigrations(
  ...
  return function deserializeFromStorage(
    eid: EntityID,
    ...
    if (!apply(eid, migrated)) {
      console.error('Failed to deserialize component data.', {
        original,
        migrated,
      });
