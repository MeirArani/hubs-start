...
export const ChatContext = createContext<ChatContextValuesT>({
  messageGroups: [],
  sendMessage: (message) => {},
});
...
function shouldCreateNewMessageGroup(
  messageGroups: NewMessageT[],
  newMessage: NewMessageT,
  now: number,
) {
  ...
  if (
    lastMessageGroup.senderSessionId !== newMessage.sessionId ||
    lastMessageGroup.sender !== newMessage.name
  ) {
    return true;
  }
...
export function ChatContextProvider({
  ...
  useEffect(() => {
    function onReceiveMessage(event: { detail: NewMessageT }) {
      ...
      if (
        ['chat', 'image', 'photo', 'video', 'permission'].includes(
          newMessage.type,
        )
      ) {
  ...
  return (
    <ChatContext.Provider
      value={{ messageGroups, unreadMessages, sendMessage, setMessagesRead }}
    >
      {children}
    </ChatContext.Provider>
  );
