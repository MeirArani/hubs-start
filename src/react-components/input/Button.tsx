...
const presetStyles = {
  ...
  transparent:
    'bg-white text-darkgrey border-white border-2 [&>svg]:text-darkgrey hover:text-darkgrey-hover hover:bg-white-hover active:bg-white-pressed',
...
export default function Button<T extends ValidTag = 'button'>({
  ...
  return (
    <Tag
      className={`min-h-12 min-w-39 font-bold transition-colors whitespace-nowrap py-0 flex items-center justify-center [&>svg]:mr-2  disabled:border-transparent disabled:cursor-not-allowed disabled:text-disabled-text disabled:opacity-70 disabled:bg-disabled ${presetStyles[preset]} ${sm ? 'h-8 min-h-8 min-w-25 py-0 px-2' : ''} ${lg || xl ? 'rounded-4xl px-12 h-16 text-base' : 'h-12 text-sm px-6'} ${lg ? 'lg:h-12' : ''} ${xl ? 'h-16' : ''} ${thin ? 'min-w-10.25 h-10.25 hover:text-text5-hover' : ''} ${thick ? 'h-13.25' : ''} rounded-base ${className}`}
      ref={ref}
      type="button"
      {...rest}
    >
